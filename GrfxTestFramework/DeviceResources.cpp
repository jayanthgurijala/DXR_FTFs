...
HRESULT DeviceResources::SaveImage(char* szPathName, void* lpBits, int w, int h)
{
    FILE* pFile = NULL;
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    // copy ray tracing output to staging resource to dump out as bmp
    {
        auto renderTarget = GetRenderTarget();
        
        //@todo we actually copy from UAV, need to figure out how to doget desc from UAV instead of RT.
        D3D12_RESOURCE_DESC outputDesc = renderTarget->GetDesc();
        D3D12_PLACED_SUBRESOURCE_FOOTPRINT footprint = {};
        UINT texNumRows = 0;
        UINT64 texRowSize = 0;
        UINT64 texSize = 0;
        auto device = GetD3DDevice();
        device->GetCopyableFootprints(&outputDesc, 0, 1, 0, &m_screenshotBufferInfo.footPrint, &texNumRows, &texRowSize, &texSize);
        m_screenshotBufferInfo.width = outputDesc.Width;
        m_screenshotBufferInfo.height = outputDesc.Height;
        m_screenshotBufferInfo.numRows = texNumRows;
        m_screenshotBufferInfo.rowSize = texRowSize;
        ...
        ThrowIfFailed(device->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&m_screenshotBufferInfo.m_stagingOutputResource)));
    }
}
